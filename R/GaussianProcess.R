...
#'
GetSigma <- function(end.time,l,sigma_f2,sigma_v2,steps){
  # two cases steps takes either one or two values.
  if(length(steps) == 1){
    grid <- seq(0,end.time,end.time/steps)
    Sigma <- matrix(NA,nrow=steps+1, ncol = steps+1)
    for (i in 1:(steps+1)){
      for(j in 1:(steps+1)){
    ...
    gridA = seq(0,end.time,end.time/steps[1]) ; gridB = seq(0,end.time,end.time/steps[2])
    Sigma <- matrix(NA,nrow=steps[1]+1, ncol = steps[2]+1)
    for (i in 1:(steps[1]+1)){
      for(j in 1:(steps[2]+1)){
        Sigma[i,j]<- covariance(gridA[i],gridB[j],sigma_f2,sigma_v2,l)
      }
    }
  }
...
#' log_pi_l with cholesky decomposition
#'
#' @param l Length scale
#' @param x Intensity function
#' @param cov_chol the cholesky decomposition of the covariance matrix
#'
#'@return Marginal of the length scale
#'
log_pi_l_chol <- function(l,x, cov_chol){
  # Need to consider the logarithm of the intensity as prior is defined on the logarithm.
  # x <- log(x) Not needed since x.cur.preproj already taken the log.
  ...
  # Next need to calculate the likelihood of the intensity given l. pi(x|l).
  x1 <- matrix(x, ncol = length(x))
  p <- ncol(x1)
  tmp <- backsolve(cov_chol, t(x1), transpose = TRUE)
  rss <- colSums(tmp ^ 2)
  log.likeli.l <- - sum(log(diag(cov_chol))) - 0.5 * p * log(2 * pi) - 0.5 * rss
...
#' Model fitting when the intensity function is assumed to have a GP prior.
#'
...
#'
#' @inheritParams mcmc_constant
#' @param steps The number of steps to partition the intensity function into.
#' @param w The under relaxed parameter takes values between -1 and 1.
#' @param prior.x The pair (sigma^2_f, 'sigma_n^2 ) of the signal variance and noise variance.
#' @param hyper.param Either the fixed value of the ISI parameter or the parameters of the Gamma prior for the ISI parameter.
#' @param sigma.h RW-metropolis variance for the ISI parameter.
#' @param start.hyper The iteration to begin inferring the ISI parameter.
#' @param hyper.start initial value of the ISI parameter.
#' @param l.param Either the fixed value of the length scale or the parameters of the Gamma prior for the length scale.
#' @param start.l The iteration to begin inferring the length scale.
#' @param sigma.l RW-metropolis variance for the length scale of the Gaussian Process.
#' @param initial.l Initial value of the length scale of the Gaussian Process.
#' @param T.min.param Either the fixed value of the refractory period or the parameters of the Gamma prior for the refractory period. If we do not want to include the refractory period set to NA.
#' @param sigma.t  RW-metropolis variance for the refractory period.
#' @param x.start The inital value for the intensity function.
#'
#' @return `iter` samples of the posterior distribution
#' @export
#'
#' @examples
#' \dontrun{
...
# Using projected process.
#' Model fitting when the intensity function is assumed to have a GP prior, using a projected process.
#'
#' @inheritParams mcmc_gp
#' @param save.every How often to save the iterations.
#' @param project the projection factor.
#' @param prior.l parameters for the gamma prior of the length scale of the Gaussian process prior.
#'
#' @return `iter` samples of the posterior distribution
#' @export
#'
...
#' \dontrun{
#'  spikes <- sort(runif(20, min=0,max=20))
#' posterior = mcmc_gp_projected(data.frame(spikes), end.time=20,
#'  iter = 10000, burn = 5000, ISI.type='Gamma',
#'  steps = 1000, w=0.0001, sigma.h = 1, sigma.l = 0.1, project = 50)
#' }
mcmc_gp_projected <- function(d.spikes, end.time, iter, burn, steps, w, prior.x =c(1000,1e-5), hyper.param = c(1,0.001), ISI.type = "Gamma", sigma.h =NA, start.hyper = 1000, hyper.start = 1, prior.l = c(1,0.01), start.l = 1000, sigma.l = NA, initial.l = 1, T.min.param = NA, sigma.t = NA, x.start = NULL, save.every = 1, project = 40){
  # Error messages
  # Check hyper.param has length 1 or 2. (for either fixed at X or prior is Gam(X,X))
  if(length(hyper.param)>2){stop("Error! The length of hyper.param != 1 or 2.\n The hyper.param you entered has length ",
                                 length(hyper.param),". \n")}
  ...
  # Setup T.min.
  {
    if(is.na(T.min.param)){
      ...
      out.T.min <- rep(NA,iter%/%save.every)
      if(is.na(sigma.t) == TRUE){stop("Error! If you want to use mcmc to get T.min estimation you require a sigma.t value.")}
  ...
  ## In this case we are inferring l (using projected process). Setup l.
  {
    if(length(prior.l) != 2 && is.numeric(prior.l)){
      stop('Prior for the length scale needs to be a vector of length 2.')
    }
    if(!(length(sigma.l) == 1 && is.numeric(sigma.l))){
      stop('Invalid sigma.l!')
    }
    ...
    if(initial.l <= 0){
      stop('initial l needs to be greater than 0.')
    }
    l.cur = initial.l
    out.l <- rep(NA,iter%/%save.every)

    # setup the covariance matrix. projection variable either the vector of values or a factor of the steps.
    if(length(project)==1 && is.numeric(project)){
      # Project is the number of steps in the smaller.
      covariance_t_tbar = GetSigma(end.time,l.cur, sigma.f2, 1e-8, steps = c(steps, project))
      covariance_tbar_tbar = GetSigma(end.time,l.cur, sigma.f2, sigma.n2, steps = project)
      chol_cov = chol(covariance_tbar_tbar)
      chol_cov_pivot <- chol(covariance_tbar_tbar, pivot = TRUE)
      chol_cov_pivot <- chol_cov_pivot[, order(attr(chol_cov_pivot, "pivot"))]
      Sig_ttb_Sig_tbtb_inv <- covariance_t_tbar %*%solve(covariance_tbar_tbar)

      # check val.
      # print(paste0('cov_t_tbar ', dim(covariance_t_tbar), ' cov_tbar_tbar = ', dim(covariance_tbar_tbar), ' Sig_ttb_Sig_tbtb_inv = ', dim(Sig_ttb_Sig_tbtb_inv)  ))
    }


  }

  ## Setup the intensity function.
  {
    t_orig <- seq(0,end.time, end.time/steps)
    avg.spikes <- sum(!is.na(d.spikes))/(ncol(d.spikes)*end.time)
    # x.cur <- rep(avg.spikes,steps+1)
    if(is.null(x.start)){
      x.cur.preproj <- log(rep(avg.spikes,project+1))
    }
    else{
      x.cur.preproj <- x.start
    }
    out.x <- matrix(NA, nrow=iter%/%save.every, ncol=steps+1)
  }

  ## Setup the ISI parameter if required.
  {
   if(length(hyper.param) == 1 && is.numeric(hyper.param)){
    hyp.cur <- hyper.param
  }
  else if(length(hyper.param) == 2 && is.numeric(hyper.param)){
    hyp.cur <- hyper.start
  }
  else{
    stop('Invalid hyper.param.')
  }
  out.hyper <- rep(NA,iter%/%save.every)
  }
  ...
  for (i in 2:(iter+burn)) {
    ...
    # Draw from the 'prior' i.e \nu \sim \mathcal{N}(\mathbf{0},\mathbf{\Sigma})
    # nu <- stat_gp(cov.fn, l = l.cur, dt.GP = end.time/steps, steps = steps)$GP
    nu <- as.numeric(rnorm(project+1) %*%  chol_cov_pivot)

    # Get the candidate x.
    x.can.preproj <- (1-w**2)**0.5 * x.cur.preproj  + w * nu
    x.can <- exp(Sig_ttb_Sig_tbtb_inv %*% x.can.preproj)
    x.cur <- exp(Sig_ttb_Sig_tbtb_inv %*% x.cur.preproj)
    ...
    # Only update every 2 (default) iterations change i%%2 == 0 if you want to update a different no of times.
    if(i > start.l && i%%10 == 0){
        # Get the candidate value for l and candidate covariance.
        l.can <- stats::rnorm(1,l.cur,sigma.l)
        covaraince.matrix.can <- GetSigma(end.time, l.cur, sigma.f2, sigma.n2, project)
        chol_cov.can <- chol(covaraince.matrix.can)

        # Calculate the likelihood.
        # Note this is calculated with a shortened intensity to aid computation time.
        log.pi.l.can <-  log_pi_l_chol(l.can,x.cur.preproj, chol_cov)
        log.pi.l.cur <-  log_pi_l_chol(l.cur,x.cur.preproj, chol_cov.can)
        # draw u from U(0,1)
        u <- stats::runif(1)
  ...
  hyp <- list(hyper = hyper.param) ; l <- list(l = out.l) ; t <- list(T.min = T.min.param)
  intensity <- list(intensity = out.x)
  l <- list(l = out.l)
  if (length(hyper.param)==2){ hyp <- list(hyper = out.hyper)}
  if (length(T.min.param)==2){ t <- list(T.min = out.T.min)}
...
#' Get mean + 95% credible of intensity function
#'
#' @param intensities matrix of posterior intensity functions
#' @param end.time The length of the experiment
#'
#' @return either plot of mean and 95% credible region. Or a list containing the mean and 95% credible region of the intensity function
#' @export
#'
cred_interval_gp <-function(intensities,end.time){
