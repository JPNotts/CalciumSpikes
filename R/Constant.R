...
#' Model fitting when the intensity function is assumed to be constant.
#'
#' This function runs an MCMC algorithm to sample from the posterior ISI distribution given calcium spike sequences, where we assume the intensity function is constant.
#'
#' @param end.time The length of the experiment
#' @param prior.hyper Either a fixed value for the ISI parameter or parameters for the gamma prior for the ISI parameter..
#' @param prior.x Either a fixed value for the intensity function or parameters for the gamma prior for the intensity function.
#' @param iter The number of iterations of the MCMC after the initial burn in.
#' @param burn The initial number of iterations to remove.
#' @param ISI.type The inhomogeneous ISI distribution. Either 'Exponential', 'Gamma', 'InverseGaussian', 'LogNormal' or 'Weibull'.
#' @param show.prog Boolean flag for do we print a progress bar of the samples.
#' @param prior.tmin prior.tmin
#' @param sigma.hyper RW-metropolis variance for the ISI parameter.
#' @param sigma.x RW-metropolis variance for the intensity function.
#' @param sigma.t.min RW-metropolis variance for the refractory period.
#' @param d.spikes The calcium spike sequences used to fit the model (a data frame where each column consists of a single spike sequence).
#'
#' @return `iter` samples of the posterior ISI distribution.
#' @export
#'
#' @examples
#' \dontrun{
#' spikes <- sort(runif(20, min=0,max=20))
#' posterior = mcmc_constant(data.frame(spikes), end.time=20, iter = 10000,
#' burn = 5000, ISI.type='Gamma', sigma.x=1, sigma.hyper = 1,prior.tmin=0)
#' }
mcmc_constant <- function(d.spikes, end.time, prior.hyper = c(1,0.01), prior.x = c(1,0.01), prior.tmin = c(1,0.01), sigma.hyper, sigma.x, sigma.t.min, iter, burn, ISI.type, show.prog=T){
  ...
  if(length(prior.tmin) == 1){
    ...
    tmin.cur = 0.3 # Set tmin.cur to very small
