...
#' Model fitting when the intensity function is assumed to have a PWC prior.
#'
...
#'
#' @inheritParams mcmc_gp
#' @param k.max Maximum number of change points in RJMCMC.
#' @param lambda Parameter of the Poisson distribution for the number of change points.
#' @param kappa parameter of priors height
#' @param mu parameter of priors height
#' @param kappa0 parameter of priors height
...
mcmc_pwc <- function(d.spikes,end.time,iter,burn, k.max, lambda, kappa, mu, kappa0,
                 ...
                 do.log = TRUE, show.iter = T, which.heights = "independent"){
...
#' get the mean and 95% credible interval of the intensity function for PWC method
#'
#' @param partitions  Matrix of posteior samples of the partition of the change points
#' @param heights Matrix of posterior heights corresponding to the PWC functions.
#' @param method Either "True" or "Approx". Where Approx only calculate the intensity values at the specified steps. True calculates at every changepoint in the posterior distribution.
#' @param steps The number of steps to partition the intensity function into.
#'
#' @return A list containing the mean and 95% credible interval for the intensity function.
#' @export
#'
cred_interval_pwc <- function(partitions, heights, method = "True", steps = 1000){
