...
#' @param steps The number of steps to partition the experiment's length.
...
#' \dontrun{
#' time <- seq(0,20,0.001)
#' int.fn <- 2*(cos(time) +1.1)
#' simulate_spikes(20,int.fn, 5,steps = 2000, ISI.type='Weibull', sequences = 10)
#' }
simulate_spikes <- function(end.time, int.fn, hyper, steps =1000, T.min = NULL, ISI.type = "Gamma", sequences = 1, add.end = TRUE,do.log = T){
  ...
  pb <- txtProgressBar(min = 0,      # Minimum value of the progress bar
                       max = sequences, # Maximum value of the progress bar
